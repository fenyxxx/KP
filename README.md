...
#### 7. 📊 Годовой отчет ППО
**Расчет плановых затрат с разбивкой по кварталам для профсоюза**
...
- **Раздел 1: Выездные мероприятия**
  ...
  - Месяц проведения
  - Затраты (руб.)
  - Распределение по кварталам (1 кв., 2 кв., 3 кв., 4 кв.)
  - Промежуточный итог по выездным
- **Раздел 2: Внутренние и городские мероприятия**
  - Аналогичная структура
  - Промежуточный итог по внутренним
- **Общий итог** с разбивкой по кварталам

**Применение:**
- Предоставление отчётности профсоюзу
- Поквартальное планирование годового бюджета ППО
- Контроль расходования средств профсоюза по периодам

#### 8. 📋 Годовой отчет УЭВП
**Расчет плановых затрат на командировочные расходы с детализацией**

**Структура отчёта:**
- **Только выездные мероприятия**
- Для каждого мероприятия:
  - Номер и название
  - Место проведения / единица измерения
  - Месяц / количество
  - Общая сумма затрат
  - Распределение по кварталам (1 кв., 2 кв., 3 кв., 4 кв.)
- **Детализация по каждому тренеру:**
  - Проезд (ж/д билеты)
  - Проживание (дней)
  - Суточные (с учётом региональных ставок)
- **Общий итог** с разбивкой по кварталам
...
### 📊 Экспорт отчётов (CSV и HTML)

**Все 8 типов отчётов** поддерживают экспорт в CSV и HTML форматы:

**📄 Экспорт в CSV:**
- Автоматическое имя файла по типу отчёта (например, `Годовой_отчет_ППО_2026.csv`)
...
- Для каждого типа отчёта - своя структура данных:
  - **Годовой отчет ППО/УЭВП:** с распределением по кварталам
  - **Финансовый отчёт:** план, факт, экономия, остаток
  - **По типам мероприятий:** разбивка внутренние/выездные
  - **И другие отчёты** - с релевантными данными
- Файл автоматически открывается в Excel

**🌐 Экспорт в HTML:**
- Готовый документ для печати и архивирования
- Корпоративный стиль "Газпром"
- Красивое оформление с таблицами и итогами
- Можно сохранить как PDF через браузер (Печать → Сохранить как PDF)
- Подходит для официальной отчётности
